        ...
        stage('sonar-scanner') {
            steps {
                withSonarQubeEnv('Sonarqube') {
                    sh "${tool('sonar-scanner')}/bin/sonar-scanner -Dsonar.projectKey=dissertation -Dsonar.projectName=courier_mgmt -Dsonar.sources=views -Dsonar.projectVersion=${BUILD_NUMBER}"
                }
            }  
        }
